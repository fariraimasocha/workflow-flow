    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex m-2 p-2">
                ...
                class="px-4 py-2 bg-fuchsia-500 hover:bg-sky-600 rounded-lg text-white">Companies</a>
            </div>
            <div class="m-2 p-2 bg-slate-100 rounded">
            <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                <form method ="POST" action="{{ route('admin.companies.update',$company->id) }}" enctype="multipart/form-data w-1/2">
                  @csrf
                  @method('PUT')
                  ...
                  <div class="sm:col-span-6">
                    ...
                    <div class="mt-1">
                      <input type="text" id="mobile"  name="mobile"  value="{{$company->mobile}}"
                      class="block w-full  appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                    </div>
                    @error('mobile')
                      <div class="alert alert-danger">{{ $message }}</div>
